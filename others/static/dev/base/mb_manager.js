var API_URL = 'https://amore-citylab.amorepacific.com:8000/v1/mb/manager/';
var page_param = {
    totalCount: 5,
    currentPage: 1,
    pageSize: 10,
...
var fnGetList = function (param) {
    // console.log('    $('#mb_manager_list').find('tr').remove();
    ajax.get(API_URL, param, function (result) {
        var list = result;
        $('#mb_manager_list').find('tr').remove();
        $.each(list, function (idx, data) {
            page_param.totalCount = data.total_count
            template.prepend($('#mb_manager_item'), $('#mb_manager_list'), data, function () {
                // data transfer here
            });
        })
        fnSetUI('list');
...
var fnValidate = function () {
    var result = $('#mb_manager_form').isValidate();
    return result;
}

var fnPost = function () {
    if (fnValidate()) {
        var param = $('#mb_manager_form').json();
        ajax.post(API_URL, param, function (result) {
            console.log('result:', result)
            $('#mb_manager_form').clear()
            fnGetList()
        });
    }
}
