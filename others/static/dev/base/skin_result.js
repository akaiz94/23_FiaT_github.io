var API_URL = 'https://amore-citylab.amorepacific.com:8000/v1/skin/result/';
var page_param = {
    totalCount: 5,
    currentPage: 1,
    pageSize: 10,
...
var fnGetList = function (param) {
    // console.log('    $('#skin_result_list').find('tr').remove();
    ajax.get(API_URL, param, function (result) {
        var list = result;
        $.each(list, function (idx, data) {
            page_param.totalCount = data.total_count
            template.prepend($('#skin_result_item'), $('#skin_result_list'), data, function () {
                // data transfer here
            });
        })
        fnSetUI('list');
...
var fnValidate = function () {
    var result = $('#skin_result_form').isValidate();
    return result;
}

var fnPost = function () {
    if (fnValidate()) {
        var param = $('#skin_result_form').json();
        ajax.post(API_URL, param, function (result) {
            console.log('result:', result)
            $('#skin_result_form').clear()
            fnGetList()
        });
    }
}
