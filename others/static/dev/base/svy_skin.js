var API_URL = 'https://amore-citylab.amorepacific.com:8000/v1/svy/skin/';
var page_param = {
    totalCount: 5,
    currentPage: 1,
    pageSize: 1000,
...
var fnGetList = function (param) {
    // console.log('    $('#svy_skin_list').find('tr').remove();
    ajax.get(API_URL, param, function (result) {
        var list = result;
        $.each(list, function (idx, data) {
            page_param.totalCount = data.total_count
            template.prepend($('#svy_skin_item'), $('#svy_skin_list'), data, function () {
                // data transfer here
            });
        })
        fnSetUI('list');
...
var fnValidate = function () {
    var result = $('#svy_skin_form').isValidate();
    return result;
}

var fnPost = function () {
    if (fnValidate()) {
        var param = $('#svy_skin_form').json();
