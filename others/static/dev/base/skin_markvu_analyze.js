var API_URL = 'https://amore-citylab.amorepacific.com:8000/v1/skin/markvu/analyze/';
var page_param = {
    totalCount: 5,
    currentPage: 1,
    pageSize: 10,
...
var fnGetList = function (param) {
    // console.log('    $('#skin_markvu_analyze_list').find('tr').remove();
    ajax.get(API_URL, param, function (result) {
        var list = result;
        $.each(list, function (idx, data) {
            page_param.totalCount = data.total_count
            template.prepend($('#skin_markvu_analyze_item'), $('#skin_markvu_analyze_list'), data, function () {
                // data transfer here
            });
        })
        fnSetUI('list');
...
var fnValidate = function () {
    var result = $('#skin_markvu_analyze_form').isValidate();
    return result;
}

var fnPost = function () {
    if (fnValidate()) {
        var param = $('#skin_markvu_analyze_form').json();
        ajax.post(API_URL, param, function (result) {
            console.log('result:', result)
            $('#skin_markvu_analyze_form').clear()
            fnGetList()
        });
    }
}
